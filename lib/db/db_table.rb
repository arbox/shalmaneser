    ...
    class DBTable
      ...
      def initialize(db_obj, # DBWrapper object
        ...
        when 'open'
          ...
          if hash['col_names']
            existing_fields = @db_obj.list_column_names(table_name).reject do |col|
              col =~ /^#{hash["addcol_prefix"]}/ || col == @index_name
            end
          ...
          raise "Parameter 'mode' needs to be either 'new' or 'open'! I got #{mode}"
        end
      end
      ...
      def list_column_names
        @db_obj.list_column_names(@table_name)
      end
      ...
      def list_column_formats
        @db_obj.list_column_formats(@table_name)
      end
      ...
      def change_format_add_columns(column_formats) # array: string*string [column_name,column_format]
        ...
        column_formats.each do |col_name, _col_format|
      ...
      #
      # returns: nothing
      # @param  column_value_pairs [Array] string*Object [column_name,column_value]
      def insert_row(column_value_pairs)
      ...
      def execute_command(command)
        ...
        LOGGER.fatal "Error executing SQL query. Command was:\n" + command
        raise e
      end
    end
  end
