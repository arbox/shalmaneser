...
module Shalmaneser
  module Rosy
    ...
    #
    # Iterator incorporates the following services:
    ...
    #
    # Setting "xwise": An "xwise" entry in the hash passed on to Iterator.new()
    ...
    class Iterator
      ...
      def initialize(ttt_obj, # RosyTrainingTestTable object
        ...
        if @splitID
          ...
          @second_table = @ttt_obj.existing_split_table(@splitID, @dataset, SplittingTask.split_index_colname)
          ...
          @standard_value_restrictions << SplittingTask.make_join_restriction(@splitID,
                                                                          @db_table,
                                                                          @dataset,
                                                                          @ttt_obj)
          ...
          @allcolnames.concat @ttt_obj.existing_split_table(@splitID, @dataset, SplittingTask.split_index_colname).list_column_names
          @allcolnames.uniq!
          ...
          @use_cols_from_second_table = [SplittingTask.split_index_colname]
        ...
        # pruning?
        if var_hash["prune"] and    # pruning requested in Iterator initialization
      ...
      def get_a_view_aux(columns,
        ...
        tables_and_cols = [DB::SelectTableAndColumns.new(@db_table, first_columns)]
        if @second_table
          tables_and_cols << DB::SelectTableAndColumns.new(@second_table, second_columns)
        end


        # and get a view
        return DB::DBView.new(tables_and_cols,
