...
public class CoarseToFineMaxRuleParser extends ConstrainedArrayParser{
	...
	boolean isBaseline;
	private final boolean doVariational;
  ...
  public CoarseToFineMaxRuleParser(Grammar gr, Lexicon lex, double unaryPenalty, int endL, 
    ...
    nLevels = (int)Math.ceil(Math.log(ArrayMath.max(numSubStatesArray))/Math.log(2));
...
*/
  		...
  		for (int start = 0; start + diff <= length; start++) {
  			...
  			for (int cState=0; cState<numStates; cState++){
    			...
    			boolean somethingChanged = false;
  				...
  				double[] scoresAfterUnaries = new double[nChildStates];
  				if (viterbi) Arrays.fill(scoresAfterUnaries,initVal);
  				for (int r = 0; r < numRules; r++) {
  					...
  					for (int cp = 0; cp < nChildStates; cp++) {
  						...
  						for (int np = 0; np < nParentStates; np++) {
  							...
  							if (viterbi) scoresAfterUnaries[cp] = Math.max(thisRound, scoresAfterUnaries[cp]); 
        				else scoresAfterUnaries[cp] += thisRound;
  							somethingChanged = true;
  						}
  					}
  				}
    			if (somethingChanged){
    				for (int cp = 0; cp < nChildStates; cp++) {
    					if (scoresAfterUnaries[cp] > initVal){
		            if (viterbi) oScore[start][end][cState][cp] = Math.max(oScore[start][end][cState][cp], scoresAfterUnaries[cp]); 
	      				else oScore[start][end][cState][cp] += scoresAfterUnaries[cp];
    					}
		        }
		      }
        }
  ...
  public void scaleArrayToScale(double[] scores, int previousScale, int newScale){
	...
	void initializeChart(List<String> sentence, Lexicon lexicon,boolean noSubstates,boolean noSmoothing,List<String> posTags) {
		int start = 0;
		int end = start+1;
...
//  		System.out.print(parse+". NoPruning: " +totalStates + ". Before: "+previouslyPossible+". After: "+nowPossible+".");
	  ...
	  public void doPreParses(List<String> sentence, Tree<StateSet> tree,boolean noSmoothing,List<String> posTags){
	  	...
	  	//Double.NEGATIVE_INFINITY;//Math.log(1.0e-10);
...
//	  	double[] pruningThreshold = {-8,-12,-12,-11,-12,-10,-9};
	  			//-15.0413970259969, -14.376490292300147, -10.912968559084675, -12.199341260578706, -10.057158130328958, -9.095949871253225
//	  	}
	  	//int startLevel = -1;
...
//	  		createArrays(level==startLevel,curGrammar.numStates,curGrammar.numSubStates,level,Double.NEGATIVE_INFINITY,false);
		    ...
		    initializeChart(sentence,curLexicon,level<1,noSmoothing,posTags);
...
//	      System.out.println("\nFound a parse for sentence with length "+length+". The LL is "+score+".");
	  ...
	  public Pair<double[][][][],double[][][][]> doInsideOutsideScores(List<String> sentence, Tree<StateSet> tree){
	  	doPreParses(sentence,tree,true,null);
	    ...
	    initializeChart(sentence,curLexicon,false,true,null);
...
//	  	computeTightThresholds(sentence);
//	  	System.out.println("\n\n\n\nTightest thresholds so far: "+Arrays.toString(maxThresholds)+"\n\n\n\n");
      ...
      maxcRightChild = null;
	    ...
	    return bestTree;//removeStars(bestTree);
	  }
	  ...
	  public double getLogLikelihood(){
	  	...
	  	Tree<String> newTree = TreeAnnotations.processTree(bestTree,1,0,binarization,false);
              ...
              //if (!allowedStates[start][end][pState]) continue;
	            ...
	            for (int i = 0; i < parentRules.length; i++) {
	              ...
	              for (int split = min; split <= max; split++) {
	                ...
	                if (leftChildScore==0||rightChildScore==0) continue;
	                //if (!allowedStates[start][split][lState]) continue;
	                //if (!allowedStates[split][end][rState]) continue;
	                for (int lp = 0; lp < nLeftChildStates; lp++) {
...
//    double[] pruningThreshold = {-6,-9.75,-10,-9.6,-9.66,-8.01,-7.4,-10};//Double.NEGATIVE_INFINITY;//Math.log(1.0e-10);
  	...
  	for (int level=startLevel; level<endLevel; level++){
	    ...
	    initializeChart(sentence,curLexicon,level<1,false,null);
	    final boolean viterbi = true, logScores = true;
	    if (level<1){
	    	doConstrainedViterbiInsideScores(curGrammar,level==startLevel); 
	    	score = viScore[0][length][0];
